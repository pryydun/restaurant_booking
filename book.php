...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!empty($errors)) {
        ...
        if (isset($_POST['table_number']) && isset($_POST['date'])) {
            header("Location: booking.php?step=3&table=" . intval($_POST['table_number']) . "&date=" . urlencode($date));
        } else {
    ...
    // Check if table is already booked for this date (one booking per day per table)
    ...
    if (!$check_stmt) {
        ...
        header("Location: booking.php?step=3&table=" . $table_number . "&date=" . urlencode($date));
        exit;
    }

    mysqli_stmt_bind_param($check_stmt, "is", $table_number, $date);
    mysqli_stmt_execute($check_stmt);
    $result = mysqli_stmt_get_result($check_stmt);

    if (mysqli_num_rows($result) > 0) {
        // Table is already booked for this date
        mysqli_stmt_close($check_stmt);
        mysqli_rollback($conn);
        $_SESSION['booking_error'] = "This table is already reserved for the selected date. Please choose another date.";
        $_SESSION['booking_form_data'] = $_POST;
